...
# ╔═╡ 07137b7f-9b24-4100-abd7-24a6f3cff2e8
begin
	# julia 中 using 与 import 都可以加载库，使用 using 时库中函数可以直接用，不用加库名
	# 若只想加载库中某个函数或变量，则使用 import Modulename: function_name, variable_name
	# 使用 ？ 或 @doc 可以调出帮助文档
	using JuMP
	using SDDP
...
# ╔═╡ 4e4f0717-109b-4abd-b1f7-7538cc2d93f4
# this is an two stage example from: https://sddp.dev/stable/tutorial/example_newsvendor/

# ╔═╡ 1fecf315-53db-4fef-ae4d-19f2d2484b6b
begin
	D = Distributions.TriangularDist(150.0, 250.0, 200.0)
	N = 100
	# 在 Julia 中，所有以 ! 结尾的函数，都表示该函数会修改其调用的参数（In-place modification)
	# 不带感叹号，则创建参数的拷贝
	d = sort!(rand(D, N)); # sort 升序
	Ω = 1:N
	P = fill(1 / N, N);
	StatsPlots.histogram(d; bins = 20, label = "", xlabel = "Demand")
end

# ╔═╡ 7cbc660b-b5f9-4f44-82f4-2956f0be2733
function kelleys_cutting_plane(
    ...
    set_silent(model) # model no output
    @variable(model, θ <= upper_bound) # add decision varaibles into the model
    @variable(model, x[1:input_dimension])
    @objective(model, Max, θ) # 最大化函数
    x_k = fill(NaN, input_dimension)
    lower_bound, upper_bound = -Inf, Inf
    while true
        # Step (2):
		# 带 ! 的函数（如 optimize!(model)）：会直接在 model 对象内部进行求解，并将结果（如最优解、状态、对偶变量）直接写入该 model 对象的内存中
		# julia optimze 基本都带！
        optimize!(model)
		# . (广播运算符)：在 Julia 中，函数名后的点号表示广播（Broadcast）。这意味着它会对 x 集合（数组或容器）中的每一个元素都调用 value() 函数。
        # value()：这是 JuMP 的函数，用于从求解器中获取变量在当前解下的具体数值
        x_k .= value.(x)
        # Step (3):
        ...
        # Step (4):
		# 在 Julia 中，如果直接写 x - x_k，要求两个数组的维度完全一致。加上点号 .- 后，它会进行逐元素（element-wise）减法
	...
	println("Found objective value: ", f(x_k))
    return
end

# ╔═╡ f850c860-194d-4c1e-914e-1419a23b3158
# ：分号后面的所有参数（input_dimension = 2, ...）会被识别为关键字参数。调用时，参数的顺序不再重要，只要名称（Key）对得上即可
kelleys_cutting_plane(;
    input_dimension = 2,
    upper_bound = 10.0,
    iteration_limit = 20,
) do x # 创建一个匿名函数，并将这个匿名函数作为左边函数的第一个参数
	# julia 数组下标从 1 开始
    return -(x[1] - 1)^2 + -(x[2] + 2)^2 + 1.0
end

# ╔═╡ 9ea44ccc-c55c-410b-a7fd-8d0dc271f0df
begin
    function solve_second_stage(x̅, d_ω)
        ...
        fix(x_in, x̅) # fix a variable to a value
        ...
        return (
            ...
            λ = reduced_cost(x_in), # 简约成本，即影子价格
            x = value(x_out),
            u = value(u_sell),
        )
    end
...
# ╔═╡ ddfa2331-9f45-420b-8d25-fb28e5d55b38
begin
	# first stage
	model = Model(HiGHS.Optimizer)
	set_silent(model)
...
# ╔═╡ d9acb85b-3d22-4894-92fd-18ad4939fd6c
begin
    kIterationLimit = 100
...
# ╔═╡ 48d7e92f-c0e2-46be-ae2d-99fd9d69680f
begin
    # solve by the SDDP library
    # SDDP tutorial: https://sddp.dev/stable/tutorial/first_steps/#An-introduction-to-SDDP.jl
    # LinearPolicyGraph，还有 MarkovPolicyGraph, LinearGraph
    model_newsvendor = SDDP.LinearPolicyGraph(;
        stages = 2,
        sense = :Max,
	...
	# log_every_iteration 输出训练的每一步结果
	# 输出结果中 pid=1 表示并行计算
	# 这个包默认使用 simulation_stopping rule: 首先 boudns 稳定，然后看out of sample value
	# 是否稳定
    SDDP.train(model_newsvendor; log_every_iteration = true)
end

# ╔═╡ c2d50c94-7001-4bae-883b-85843ba8d88e
first_stage_rule = SDDP.DecisionRule(model_newsvendor; node = 1)

# ╔═╡ a6e15af4-3e26-42d8-9c68-f0a19f201a44
solution_1 = SDDP.evaluate(first_stage_rule; incoming_state = Dict(:x => 0.0))

# ╔═╡ defb4100-b6fa-445f-a822-ff0842a06c25
begin
	second_stage_rule = SDDP.DecisionRule(model_newsvendor; node = 2)
...
# ╔═╡ 6afe245e-4dba-4ccd-b17a-6d25f9d9f707
...
# ╔═╡ 6da4d833-f941-48e5-b686-c9e1099870a3
begin
...
# ╔═╡ c5332fba-20d2-4fb3-833e-92dd3e33327a
...
# ╔═╡ 85f3a342-30b1-4655-a637-bd634545dbaa
solve_newsvendor(SDDP.CVaR(0.4))

# ╔═╡ f16502d7-2e68-4b6d-9bf2-31e0bcd76d14
solve_newsvendor(SDDP.WorstCase())

# ╔═╡ 2de9d224-b365-49f4-bd57-933539d2931c
begin
...
manifest_format = "2.0"
project_hash = "49fd3c8cccee904265c48897faa1384c801830d1"
...
git-tree-sha1 = "df12db67b9dd45244e35a86bdca2400754de6097"
